...
function insert_tab(name)
  local btn_uri = "$frame/guild/personal_info.xml"
  local btn_sty = "tab_btn"
  local page_uri = "$frame/guild/personal_info.xml"
  local page_sty = name
  ui_widget.ui_tab.insert_suit(w_desc, name, btn_uri, btn_sty, page_uri, page_sty)
  local btn = ui_widget.ui_tab.get_button(w_desc, name)
...
function update_personal_info()
  ...
  g_p_pos.text = bo2.gv_guild_title:find(my_info.title).name
  g_p_wel.text = my_info.welfare
  local wel = bo2.gv_guild_welfare:find(my_info.welfare + 1)
  if wel == nil then
    g_p_needcontri.text = L("")
  else
    local needcon = wel.totalc - my_info.total_con
    local v = sys.variant()
    v:set("num", needcon)
  ...
  g_p_btn_newpos.enable = false
  for i = 0, bo2.gv_guild_title.size - 1 do
    local line = bo2.gv_guild_title:get(i)
    if line.status >= my_info.guild_pos and line.count == 0 and line.id ~= my_info.title and line.needcon <= my_info.total_con and line.needcon ~= 0 then
      g_p_btn_newpos.enable = true
      break
    end
  end
  g_p_ucon.text = my_info.current_con
  g_p_wlimit.text = get_week_con_max(my_info.welfare)
  g_p_wsalary.money = my_info.salary
  if my_info.salary ~= 0 then
    g_p_btn_getsal.enable = true
    g_p_btn_getsal.text = ui.get_text("guild|p_info_btn_getsal")
  else
    if my_info.salary_flag == 1 then
      g_p_btn_getsal.text = ui.get_text("guild|p_info_btn_gotsal")
    else
      g_p_btn_getsal.text = ui.get_text("guild|p_info_btn_getsal")
    end
    g_p_btn_getsal.enable = false
  end
end
function on_apply_pos(btn)
  ui_guild_mod.ui_member.on_title_apply()
end
    ...
    if excel.in_theme == bo2.eThemeType_Guild and order ~= 0 then
      ...
      elseif ui.quest_check_insert(excel.id) and bo2.is_cooldown_over(excel.cooldown) then
        for j = 0, excel.guild_quest_jiaopai.size - 1 do
          if ui.guild_has_scn() then
            if 1 == excel.guild_quest_jiaopai[j] then
              quest_order[order] = excel.id
            end
          elseif 4 == excel.guild_quest_jiaopai[j] then
            quest_order[order] = excel.id
          end
        end
      end
  ...
  if not excel.awd_match_level.empty then
    ...
    if g_awd_match_level ~= nil then
      ...
      if 0 < tbl.awd_money then
        ...
        item_money.size = ui.point(300, 20)
      ...
      for i = 0, 3 do
        ...
        if ui_quest.is_guild_obj(awd_obj) then
          ...
          has_reward = true
        elseif awd_obj == bo2.eQuestObj_ReputePoint then
    ...
    for i = 0, 3 do
      if ui_quest.is_guild_obj(excel.awd_obj[i]) then
        local item_guild = w_rewards_list:item_append()
        item_guild:load_style(rewards_uri, "guild_reward")
        item_guild.size = ui.point(300, 20)
        local box = item_guild:search("box")
...
function on_quest_num_update(cmd, data)
  if ui.npc_guild_mb_id() ~= 0 then
    ui_npc_guild_mod.ui_npc_guild_personal.handle_update_quest_num()
  else
    handle_update_quest_num()
  end
end
function handle_update_quest_num()
  ...
  local build = bo2.gv_build_level:find(guild_level + 100)
  local total_num = 0
  if build ~= nil then
    total_num = build.quest_num
  end
  if ui.guild_worldscn_id() ~= 0 then
...
local sig = "ui_guild_mod.ui_guild_personal:on_signal"
reg(packet.eSTC_UI_ShowQuestNum, on_quest_num_update, sig)
