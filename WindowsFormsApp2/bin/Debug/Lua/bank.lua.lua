c_text_item_file = L("$frame/bank/bank.xml")
...
function get_visible()
  local w = ui.find_control("$frame:bank")
  return w.visible
end
function show_bank()
  set_visible(true)
  ui_item.set_visible(true)
  ui_newbank.set_visible(false)
end
function close_bank()
  set_visible(false)
...
function get_useable_grid(excel_id)
  local size = bo2.get_bank_cur_size()
  local w_box = w_root:search(sys.format("box:%d", bo2.eItemBox_Bank))
  local w_cells = w_box:search("w_cells")
  for i = 0, size - 1 do
    local cell = w_cells:search(sys.format("grid:%d", i))
    local card = cell:search("card")
    local info = ui.item_of_coord(bo2.eItemBox_Bank, i)
    if info == nil then
      return i, card
    end
...
function on_click_tidy_box()
  ui_item.send_tidy(bo2.eItemBox_Bank)
  bo2.PlaySound2D(522)
end
function on_init_box(ctrl, data)
  ...
  for r = 0, c_box_size_y - 1 do
    for i = 0, c_box_size_x - 1 do
      ...
      local b_size = bo2.get_bank_cur_size()
      if grid >= b_size then
        d.enable = false
...
function on_card_mouse(card, msg, pos, wheel)
  ...
  elseif msg == ui.mouse_rbutton_click then
    ...
    local on_msg = function(msg)
      ...
      ui_item.send_bank(msg.only_id, -1, -1, count)
    end
    ...
    elseif info.count == 1 then
      ui_item.send_bank(info.only_id, -1, -1, info.count)
    end
  end
end
...
function get_max_bank_size()
  local e_size = bo2.gv_bank_extend.size
  local b_excel = bo2.gv_bank_extend:get(e_size - 1)
  local i_excel = bo2.gv_item_list:find(b_excel.item_id)
