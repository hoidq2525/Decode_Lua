...
function on_tip_show_product(tip)
  ...
  local info = {
    ...
    get_identify_state = function()
      return bo2.eIdentifyEquip_Finish
    end
  }
...
function on_lock_btn_tip(tip)
  ...
  ui_tool.ctip_push_text(stk, ui.get_text("equip|tip_before_trait"), ui_tool.cs_tip_color_green, ui_tool.cs_tip_a_add_m)
  ...
  ui_tool.ctip_push_sep(stk)
  ui_tool.ctip_push_text(stk, ui.get_text("equip|tip_after_trait"), ui_tool.cs_tip_color_green, ui_tool.cs_tip_a_add_m)
  ui_tool.ctip_push_sep(stk)
...
function on_equip_change(card)
  ...
  local item_file = "$frame/npcfunc/tianwu_upgrade.xml"
  local item_style = "trait_item"
  local fix_traits = excel.fix_traits
  local fix_traits_post = pdt_excel.fix_traits
  local cnt = fix_traits.size
  local cnt_post = fix_traits_post.size
  if cnt == cnt_post then
    for i = 0, cnt - 1 do
      local id = fix_traits[i]
      local post_id = fix_traits_post[i]
      local item = w_pro_list:item_append()
      item:load_style(item_file, item_style)
      local t_color = ui.make_color(ui_tool.cs_tip_color_green)
      if id > 0 then
        local property_name = item:search("property_text")
        property_name.text = ui_tool.ctip_trait_text(id)
        property_name.color = t_color
      end
      if post_id > 0 then
        local post_trait = item:search("post_trait")
        post_trait.text = ui_tool.ctip_trait_text(post_id)
...
function on_equip_drop(pn, msg, pos, data)
  ...
  local upgrade_line = get_equip_upgrade_line(info.excel_id)
  if upgrade_line == nil or upgrade_line.is_tianwu == 0 then
...
function on_upgrade_click()
  ...
  local v = sys.variant()
  v:set(packet.key.talk_excel_id, bo2.eNpcFunc_TianwuUpgrade)
