...
function on_visible(w, vis)
  ...
  if vis == false then
    ...
    ui.item_mark_show("ridepet_weapon_skill_up", false)
  else
    ui.item_mark_show("ridepet_weapon_skill_up", true)
  end
end
...
function on_update_skill_state()
  ...
  if w_equip_target.info ~= nil and w_equip_material.info ~= nil then
    update_skill_lock(w_equip_target, w_equip_material)
...
function get_preview_money_enable()
  ...
  local skill_id = ui_npcfunc.ui_ridepet_cmn.get_select_skill_id(w_equip_target)
  if skill_id == 0 then
    return text, money, enable
  end
  local target_info = w_equip_target.info
  local target_skill_idx = ui_npcfunc.ui_ridepet_cmn.find_skill_idx(target_info, skill_id)
  local target_level = target_info:item_get_ridepet_skill_level(target_skill_idx)
  local skill_excel = bo2.gv_ridepet_skill:find(skill_id)
  if target_level >= skill_excel.nMaxLevel then
    text = ui.get_text("npcfunc|equip_ridepet_skill_exp_full")
    return text, money, enable
  end
  ...
  local skill_exp_excel = bo2.gv_equip_ridepet_skill_exp:find(material_level)
  if skill_exp_excel == nil then
    return text, money, enable
  end
  local arg = sys.variant()
  arg:set("percent", skill_exp_excel.nPercent[target_level])
  text = sys.mtf_merge(arg, ui.get_text("npcfunc|equip_ridepet_skill_exp_text"))
  money = skill_exp_excel.nMoney[target_level]
  enable = true
  return text, money, enable
end
...
function on_target_mouse(card, msg, pos, wheel)
  ...
  ui_npcfunc.ui_ridepet_cmn.select_skill(w_equip_target, card_skill)
  on_update_preview()
end
function on_btn_ok()
  local v = sys.variant()
  v:set(packet.key.talk_excel_id, bo2.eNpcFunc_RidePetWeaponAddSkillExp)
