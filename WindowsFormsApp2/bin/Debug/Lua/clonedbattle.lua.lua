local item_uri = L("$frame/clonedbattle/clonedbattle.xml")
...
local cwstr_cell_style_uri = L("$frame/clonedbattle/clonedbattle.xml")
...
function on_timer_set_self_time()
  local item_fight_state = ui_cloned_battle.w_self_info:search("lb_challenge_state")
...
function on_refresh_self_data_tips()
  ...
  ui_cloned_battle.w_self_info.tip.text = stk.text
end
...
function on_init_public_info_text()
  ...
  local on_get_current_text = function()
...
function refresh_token_time()
  ...
  local times_text = sys.format("<c+:C92B2B>%d<c->", g_cloned_battle_challenge_token)
  local mtf_text = sys.format("<c+:B4B4B4>%s<c->", ui.get_text("clonedbattle|today_token"))
  g_challenge_times.mtf = ui_widget.merge_mtf({times = times_text}, mtf_text)
        ...
        if day == 1 then
          ...
          if cur_hour >= 12 then
            ...
            return ui_widget.merge_mtf({
              hour = get_full_time(23 - cur_hour),
...
function set_cell_data(cell, friend_data)
  ...
  if friend_data ~= nil then
    ...
    label_level.text = "Lv" .. friend_data.atb[bo2.eAtb_Level]
