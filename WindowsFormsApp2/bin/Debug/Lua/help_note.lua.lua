...
local c_span_fader1 = 10
local c_span_fader2 = 500
local c_span_delay = 100
local c_span = c_span_fader1 + c_span_fader2 + c_span_delay
local c_span_min = c_span - 20
local c_span_max = c_span + 160
c_limit_group_help = 10
...
function reset_fader(d, span, limit_group)
  local f = d.fader
  f:reset(0, 1, 120)
  f.visible = true
  d.tick = sys.tick()
...
function insert(w, text, color, span, show_scn, limit_group, limit_count)
  ...
  reset_fader(item_data, span, limit_group)
  local mtf
  if color ~= nil then
