...
function on_ok()
  local v = sys.variant()
  v:set(packet.key.talk_excel_id, bo2.eNpcFunc_AssRefine)
...
function on_card_chg(card, onlyid, info)
  ...
  local index
  for i = 0, ass_t_excel.grow_prop_level.size do
    if ass_t_excel.grow_prop_level[i] == level then
      index = i
    end
  end
  if index == nil then
  ...
  local prize = prize_excel.refine_prize[index]
