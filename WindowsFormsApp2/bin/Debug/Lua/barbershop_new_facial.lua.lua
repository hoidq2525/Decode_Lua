...
local cwstr_style_uri = L("$frame/personal/barbershop_plus_facial.xml")
...
local view_edit_scn = {}
view_edit_scn[1] = {scn_id = 1001, fov = 1}
view_edit_scn[2] = {scn_id = 1002, fov = 0.8}
...
function on_click_close(btn)
  local w = ui.find_control("$frame:barbershop_facial")
  w.visible = false
end
function on_click_show_edit_core(btn)
  local p_edit = ui_barbershop.ui_facial.w_panel_edit
  p_edit.visible = true
  btn.visible = false
end
function on_esc_stk_visible(w, vis)
  ...
  local function set_scn_view(vis)
    if vis then
      ui_barbershop.ui_facial.w_scn.visible = true
      ui_barbershop.ui_facial.w_scn:set_excel_id(ciScnViewIdx)
      local player = bo2.player
      bind_player(player)
      on_self_hat(player)
      on_self_body(player)
      ...
      ui_barbershop.ui_facial.w_scn.visible = true
      ui_barbershop.ui_facial.w_scn:set_excel_id(0)
      p_barbershop_player = nil
    end
  end
  if vis then
    ...
    if btn_choose.enable == true then
      btn_choose = w_barbershop_control:search("portrait"):search("btn_choose")
    end
    on_click_show_edit_panel(btn_choose)
  else
    set_scn_view(vis)
    if ui_barbershop.ui_facial.w_panel_edit ~= nil then
      ui_barbershop.ui_facial.w_panel_edit.visible = false
      set_tiny_scn_view(0)
    end
    local obj = bo2.player
    if obj ~= nil then
...
function on_set_confirm_item_card(item_panel, card_panel, text_panel, data)
  ...
  if item_get_count >= cn_default_item_num then
    ...
    image.image = sys.format(L("$icon/item/%s.png"), pItemExcel.icon)
    image.visible = true
...
function refresh_current_highlight(idx)
  ...
  local w_edit_panel = ui_barbershop.ui_facial.w_panel_edit:search(name)
...
function on_view_card_value(_excel_id, card)
  ...
  if _excel_id ~= nil and barbershop_list[_excel_id] ~= nil then
    ...
    if _type == 1 then
      ...
      local text_panel = ui_barbershop.ui_facial.panel_hair_card:search(cwstr_confirm_item_text)
      on_set_confirm_item_card(ui_barbershop.ui_facial.panel_hair_card, ui_barbershop.ui_facial.confirm_hair_card, text_panel, _excel_data)
    ...
    elseif _type == 2 then
      ...
      local text_panel = ui_barbershop.ui_facial.panel_face_card:search(cwstr_confirm_item_text)
      on_set_confirm_item_card(ui_barbershop.ui_facial.panel_face_card, ui_barbershop.ui_facial.confirm_face_card, text_panel, _excel_data)
      p_barbershop_player:set_view_equip(_set_type, _excel_data._data)
    elseif _type == 3 then
      barbershop_list._selfRef.view_hair_color = _excel_data
      local text_panel = ui_barbershop.ui_facial.panel_hair_color_card:search(cwstr_confirm_item_text)
      on_set_confirm_item_card(ui_barbershop.ui_facial.panel_hair_color_card, ui_barbershop.ui_facial.confirm_hair_color_card, text_panel, _excel_data)
      p_barbershop_player:set_hair_color(_excel_data._hex_data)
    elseif _type == 4 then
      barbershop_list._selfRef.view_portrait = _excel_data
      local text_panel = ui_barbershop.ui_facial.panel_portrait_card:search(cwstr_confirm_item_text)
      on_set_confirm_item_card(ui_barbershop.ui_facial.panel_portrait_card, ui_barbershop.ui_facial.confirm_portrait_card, text_panel, _excel_data)
    end
  end
end
function on_clear_confirm_panel_value(card_panel)
  ...
  local w_edit_panel = ui_barbershop.ui_facial.w_panel_edit:search(panel_name)
  local view = w_edit_panel:search("view")
  for i = 0, n_page_limit - 1 do
    local cell_name = sys.format(cwstr_cell_name, i)
    local cell = view:search(cell_name)
...
function on_canel_view_card_value(_excel_id)
  if _excel_id ~= nil and barbershop_list[_excel_id] ~= nil then
    ...
    if _type == 1 then
      ...
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_hair_card)
      on_click_update_hair_color_view()
    elseif _type == 2 then
      p_barbershop_player:set_view_equip(bo2.eEquipData_Face, 0)
      barbershop_list._selfRef.view_face = nil
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_face_card)
    elseif _type == 3 then
      p_barbershop_player:set_hair_color(0)
      barbershop_list._selfRef.view_hair_color = nil
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_hair_color_card)
    elseif _type == 4 then
      barbershop_list._selfRef.view_portrait = nil
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_portrait_card)
    end
    on_set_total_money()
  end
end
function on_mouse_barbershop_card_canel_value(_excel_id, card)
  if _excel_id ~= nil and barbershop_list[_excel_id] ~= nil then
    ...
    local _set_type = 0
    if _type == 1 then
      local icon_card = ui_barbershop.ui_facial.panel_hair_card:search("barbershop_icon")
      ...
      on_click_update_hair_color_view()
    elseif _type == 2 then
      local icon_card = ui_barbershop.ui_facial.panel_face_card:search("barbershop_icon")
      if icon_card.excel_id ~= _excel_id then
        return false
      end
      refresh_panel_select_hightlight(card, false)
      ...
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_face_card)
    elseif _type == 3 then
      local icon_card = ui_barbershop.ui_facial.panel_hair_color_card:search("barbershop_icon")
      if icon_card.excel_id ~= _excel_id then
        return false
      end
      refresh_panel_select_hightlight(card, false)
      ...
      on_clear_confirm_panel_value(ui_barbershop.ui_facial.panel_hair_color_card)
    elseif _type == 4 then
      local icon_card = ui_barbershop.ui_facial.panel_portrait_card:search("barbershop_icon")
      if icon_card.excel_id ~= _excel_id then
        return false
      end
      refresh_panel_select_hightlight(card, false)
...
function on_req_use_card(_excel_id)
  ...
  local function on_msg_callback(msg)
    ...
    if ui_barbershop.ui_facial.w_main.visible ~= true then
      return false
    end
    SendChgEquipModelPacket(msg._excel_id)
  end
  local ref_cast = {
    item1 = _excel_data.cast_item_id,
    money = _excel_data.cast_money
  }
  local mtf_text = ui_widget.merge_mtf(ref_cast, sys.format(ui.get_text("personal|barbershop_single_msg_text")))
  local msg = {
...
function on_barbershop_confirm(btn)
  ...
  local _mtf_text = ui_widget.merge_mtf(ref_cast, sys.format(ui.get_text("personal|barbershop_single_msg_text")))
  if ui_barbershop.ui_facial.w_main.visible ~= true then
    return false
  end
  local view_hair = barbershop_list._selfRef.view_hair
...
function on_hair_card_show(tip)
  ...
  ui_tool.ctip_push_operation(stk, ui.get_text("personal|barbershop_lclick"))
  ui_tool.ctip_push_operation(stk, ui.get_text("personal|barbershop_rclick"))
  ui_tool.ctip_show(tip.owner, stk, stk_use)
  local tip = card.tip
  local view = tip.view
  local sz = ui_main.w_top.size
  local view_sz = view.size
  local window_dx = ui_barbershop.ui_facial.w_main.dx
  local window_dy = ui_barbershop.ui_facial.w_main.dy
  local window_x = ui_barbershop.ui_facial.w_main.x
  local window_y = ui_barbershop.ui_facial.w_main.y
  local view_off = ui.point(window_x + window_dx - 55, window_y + 33)
...
function on_self_enter(obj, msg)
  obj:insert_on_flagmsg(bo2.eFlagType_Int8, bo2.ePlayerFlagInt8_Body, on_self_body, "ui_barbershop.ui_facial.on_self_body")
  obj:insert_on_flagmsg(bo2.eFlagType_Int8, bo2.ePlayerFlagInt8_Hat, on_self_hat, "ui_barbershop.ui_facial.on_self_hat")
end
bo2.insert_on_scnmsg(bo2.eScnObjKind_Player, bo2.scnmsg_self_enter, on_self_enter, "ui_barbershop.ui_facial.on_self_enter")
